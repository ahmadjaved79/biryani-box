...
import {
  ...
  Utensils, CreditCard, X, Plus, Minus, Sparkles,
} from 'lucide-react';
...
const CustomerOverview = ({ profile }) => {
  ...
  const progress = tc.next
    ? Math.min(100, ((profile?.loyalty_points || 0) / tc.next) * 100)
    : 100;
  ...
  const actions = [
    {
      ...
      desc:  cartCount > 0 ? `${cartCount} items` : 'Empty',
      onClick: () => navigate('/cart'),
      active: cartCount > 0,
    },
    {
      icon: Utensils, label: 'Order Now',
      desc: 'Browse full menu',
      onClick: () => navigate('/#menu'),
      active: true,
    },
    {
      icon: RotateCcw, label: 'Reorder',
      desc: lastOrder ? 'Repeat last order' : 'No previous orders',
      onClick: () => navigate('/customer/history'),
      active: !!lastOrder,
    },
    {
      icon: Clock, label: 'Reservations',
      desc: 'Book a table',
...
const OrderTracker = ({ order }) => {
  if (!order) return (
    <EmptyState icon={Package} text="No active order. Place an order from the menu." />
  );
...
const Recommendations = ({ items, onAddToCart }) => {
  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide sm:grid sm:grid-cols-3 sm:overflow-visible">
      {items.slice(0,6).map(item => <FoodCard key={item.id} item={item} onAddToCart={onAddToCart} size="lg" />)}
    </div>
  );
};
const _OldRecommendations = ({ items, onAddToCart }) => {
...
const ActiveOffers = ({ offers }) => {
  ...
  const typeStyle = {
    new_customer: 'border-green-500/30 bg-green-500/5',
    loyalty:      'border-primary/30 bg-primary/5',
    freebie:      'border-purple-500/30 bg-purple-500/5',
    ...
    <div className="grid grid-cols-1 gap-4">
      {offers.map(offer => (
        <div key={offer.id} className={`rounded-xl border p-4 overflow-hidden ${typeStyle[offer.type] || typeStyle.announcement}`}>
          ...
          {offer.min_order > 0 && (
            <p className="text-[10px] text-text-muted">Min. order: {fmt(offer.min_order)}</p>
          )}
          {offer.code && (
          ...
          {offer.expires && (
            <p className="text-[9px] text-text-muted/50 mt-2">Expires {fmtDate(offer.expires)}</p>
          )}
        </div>
      ))}
    </div>
...
const CartSnapshot = ({ cart, total, updateQuantity, removeFromCart, navigate }) => {
  if (!cart.length) return (
    <EmptyState icon={ShoppingCart} text="Your cart is empty. Explore the menu to add items." />
  );
          ...
          <div key={order.id} className="bg-white/3 border border-white/8 rounded-xl overflow-hidden">
            <button
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-text-muted text-[10px]">{fmtDate(order.created_at)}</p>
              </div>
            </button>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                  ...
                  <div className="px-4 sm:px-5 pb-4 border-t border-white/5 pt-3">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button
                        onClick={() => window.open(`/feedback?order=${order.id}`, '_self')}
                        className="flex items-center gap-1.5 px-4 py-2 bg-white/5 hover:bg-white/10 text-text-muted border border-white/10 rounded-lg text-xs font-bold transition-all">
...
const FeedbackWidget = ({ user }) => {
  ...
  if (done) return (
    <div className="flex flex-col items-center gap-3 py-8 text-center">
      ...
      <p className="text-text-muted text-sm">Our team will review it shortly.</p>
      <button onClick={() => setDone(false)} className="text-primary text-xs hover:underline mt-2">Submit another</button>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map(s => (
          <button key={s} onMouseEnter={() => setHover(s)} onMouseLeave={() => setHover(0)}
            ...
            <Star size={22} className={`transition-all ${s <= (hover || form.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-white/20'}`} />
          </button>
        ))}
      </div>
      ...
      <textarea
        ...
        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-text-muted focus:outline-none focus:border-primary resize-none"
      />

      <button onClick={submit} disabled={loading || !form.message.trim()}
        className="btn-primary px-6 py-2.5 flex items-center gap-2 text-sm font-bold disabled:opacity-50">
        {loading ? <Loader size={15} className="animate-spin" /> : <MessageSquare size={15} />}
        Submit Feedback
      </button>
    </div>
  );
      ...
      <div className={`rounded-2xl border ${tc.border} ${tc.bg} p-5 text-center overflow-hidden`}>
        ...
        {benefits.nextTier && (
          <p className="text-text-muted text-xs mt-3">
            {benefits.pointsNeeded > 0 ? `${benefits.pointsNeeded} pts to reach ${benefits.nextTier}` : `You've reached ${tier}!`}
          </p>
        )}
        {benefits.discount > 0 && (
          <div className="mt-4 inline-block px-4 py-2 bg-primary/10 border border-primary/20 rounded-full text-primary text-sm font-bold">
            {benefits.discount}% discount active on all orders
          </div>
        )}
      </div>
...
// ── Food image card backgrounds (emoji-based gradient covers) ──────────────────
const FOOD_GRADIENTS = {
  'Biryani':     'linear-gradient(135deg,#7c3aed,#4338ca)',
  'Appetizers':  'linear-gradient(135deg,#dc2626,#f97316)',
  'Dessert':     'linear-gradient(135deg,#db2777,#9333ea)',
  'Combos':      'linear-gradient(135deg,#0891b2,#1d4ed8)',
  'Drinks':      'linear-gradient(135deg,#059669,#0891b2)',
};

const FoodCard = ({ item, onAddToCart, size = 'md' }) => {
  const bg = FOOD_GRADIENTS[item.category] || 'linear-gradient(135deg,#374151,#111827)';
  const isLg = size === 'lg';
  return (
    <motion.div
      whileHover={{ scale:1.03, y:-4 }}
      whileTap={{ scale:0.98 }}
      className={`relative rounded-2xl overflow-hidden cursor-pointer flex-shrink-0 ${isLg ? 'w-52 h-64 sm:w-56 sm:h-72' : 'w-40 h-52 sm:w-44 sm:h-56'}`}
      style={{ background: bg, boxShadow:'0 8px 32px rgba(0,0,0,0.4)' }}
    >
      {/* Giant emoji */}
      <div className="absolute inset-0 flex items-center justify-center">
        <span className={`${isLg ? 'text-7xl' : 'text-5xl'} filter drop-shadow-2xl`} style={{transform:'rotate(-10deg)'}}>
          {item.image_emoji || '🍽️'}
        </span>
      </div>
      {/* Overlay gradient */}
      ...
      {/* Veg/Halal badge */}
      {item.is_veg && (
        <div className="absolute top-2 left-2 w-5 h-5 border-2 border-green-500 rounded-sm flex items-center justify-center bg-black/50">
      ...
      {item.rating && (
        <div className="absolute top-2 right-2 flex items-center gap-1 bg-black/60 rounded-full px-2 py-0.5">
          ...
          <span className="text-yellow-400 text-[10px] font-bold">{parseFloat(item.rating).toFixed(1)}</span>
        </div>
      )}
      {/* Bottom info */}
      <div className="absolute bottom-0 left-0 right-0 p-3">
        <p className="text-white font-black text-sm leading-tight mb-0.5 line-clamp-2">{item.name}</p>
        <p className="text-white/60 text-[10px] mb-2">{item.category}</p>
        <div className="flex items-center justify-between">
          <p className="text-primary font-black text-base">{fmt(item.price)}</p>
          <motion.button
            whileTap={{ scale:0.9 }}
            onClick={e => { e.stopPropagation(); onAddToCart(item); }}
            className="w-7 h-7 bg-primary rounded-full flex items-center justify-center text-white font-black text-lg leading-none hover:bg-primary-hover transition-all shadow-lg"
          >
            +
          </motion.button>
        </div>
...
const PopularNow = ({ onAddToCart }) => {
  ...
  if (loading) return (
    <div className="flex gap-4 overflow-x-auto pb-3 sm:pb-0 sm:grid sm:grid-cols-4">
      {[...Array(4)].map((_,i) => (
        <div key={i} className="w-44 h-56 flex-shrink-0 bg-white/5 rounded-2xl animate-pulse sm:w-auto"/>
      ))}
    </div>
  );
  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide sm:grid sm:grid-cols-4 sm:overflow-visible">
...
const CustomerDashboardPage = () => {
  ...
  const handleReorder = (items) => {
    items.forEach(item => addToCart({
      id: item.menu_item_id || Math.random(),
    ...
    <div className="min-h-screen bg-bg-main text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-20 sm:pt-24 pb-20 space-y-4 sm:space-y-6 w-full overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <CustomerOverview profile={profile} />
        </motion.div>

        {/* 2 · Quick Actions */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Section title="Quick Actions" icon={Flame}>
            <QuickActions navigate={navigate} cartCount={cart.length} lastOrder={orderHistory[0]} />
          </Section>
        </motion.div>

        {/* 3 · Active Order Tracking */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <Section title="Order Tracking" icon={MapPin}
            action={activeOrder && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
            <Section title="Your Offers" icon={Tag}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Section title={`Cart (${cart.length})`} icon={ShoppingCart}
            action={cart.length > 0 && (
              <button onClick={() => navigate('/checkout')} className="text-xs text-primary hover:underline flex items-center gap-1">
                Checkout <ChevronRight size={11} />
              </button>
            )}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}>
            <Section title="Quick Feedback" icon={MessageSquare}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <Section title="Popular Right Now" icon={TrendingUp}>
            <PopularNow onAddToCart={addToCart} />
          </Section>
        </motion.div>
      </div>
    </div>
