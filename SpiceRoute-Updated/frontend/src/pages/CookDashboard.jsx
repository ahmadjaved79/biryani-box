...
import { ordersAPI, ingredientsAPI, notificationsAPI } from '../api/index.js';
...
const CookDashboard = () => {
  ...
  const handleRestock = async (id, amount) => {
    try {
      await ingredientsAPI.restock(id, amount);
      setIngredients(prev => prev.map(i => i.id === id ? { ...i, stock: parseFloat(i.stock) + parseFloat(amount) } : i));
      showAlert('Stock updated!');
    } catch { showAlert('Restock failed', 'error'); }
  };
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      {[5, 10, 25].map(amount => (
                        <button
                          key={amount}
                          onClick={() => handleRestock(ing.id, amount)}
                          className="flex-1 py-1.5 bg-primary/20 hover:bg-primary text-primary hover:text-white border border-primary/30 rounded-lg text-xs font-bold transition-all"
                        >
                          +{amount}
                        </button>
                      ))}
