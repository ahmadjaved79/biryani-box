...
import { User, Lock, Mail, Phone, Loader, ShieldCheck, ChefHat } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/useContextHooks';
import Navbar from '../components/Navbar';

const CustomerAuth = () => {
  const { customerLogin, customerRegister, loading, authError } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'login') {
      ...
      result = await customerRegister(form.name, form.email, form.password, form.phone);
    }
    if (result.success) {
      ...
      <div className="flex items-center justify-center min-h-screen px-6 pt-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {(error || authError) && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 text-red-400 text-sm">{error || authError}</div>
            )}

            <button type="submit" disabled={loading}
              className="btn-primary w-full py-4 flex items-center justify-center gap-2 text-sm font-bold uppercase disabled:opacity-60">
          ...
          <div className="mt-6 text-center space-y-2">
            ...
            <button onClick={() => navigate('/login')} className="text-xs text-text-muted hover:text-primary transition-all">
              Staff Login →
            </button>
          </div>
        </motion.div>
