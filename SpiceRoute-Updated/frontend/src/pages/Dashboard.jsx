...
/* ══════════════════════════════════════════════════
   DESIGN TOKENS — only change here to retheme
══════════════════════════════════════════════════ */
const CSS = {
  // Sidebar
  sidebar: `
  ...
  navItem: (active) => `
    ...
    ${active
      ? 'bg-gradient-to-r from-[#e63946] to-[#c1121f] text-white shadow-lg shadow-red-900/30'
  ...
  input: `
    ...
    focus:outline-none focus:border-[#e63946]/60 focus:bg-white/[0.06]
  ...
  btnPrimary: `
    ...
    bg-gradient-to-r from-[#e63946] to-[#c1121f] text-white
    hover:from-[#ff4757] hover:to-[#e63946]
    shadow-lg shadow-red-900/20 hover:shadow-red-900/40
  ...
  badge: (color) => {
    const map = {
      ...
      gray:   'bg-white/5 border-white/20 text-[#9ca3af]',
      primary:'bg-[#e63946]/10 border-[#e63946]/30 text-[#e63946]',
    };
...
const Sidebar = ({ activeTab, setActiveTab, user, unreadCount, mobileOpen, setMobileOpen }) => {
  ...
  return (
    <>
      ...
      <aside className={`
        ${CSS.sidebar}
        ${mobileOpen ? CSS.sidebarMobileVisible : CSS.sidebarMobileHidden}
        lg:translate-x-0
      `}>
        {/* Logo */}
        <div className="px-4 py-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-gradient-to-r from-[#e63946]/10 to-transparent border border-[#e63946]/20">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-[#e63946] to-[#c1121f] flex items-center justify-center shadow-lg shadow-red-900/40 flex-shrink-0">
              <Command size={16} className="text-white" />
            </div>
            <div>
              <p className="text-white font-black text-sm tracking-widest">SPICEROUTE</p>
              <p className="text-[#e63946] text-[9px] font-bold uppercase tracking-[0.3em]">System v2</p>
            </div>
          </div>
        </div>
          ...
          {items.map(item => (
            ...
            >
              <div className="flex items-center gap-3">
                <item.icon
                  size={15}
                  className={activeTab === item.id ? 'text-white' : 'text-[#e63946]/50'}
                />
                {item.label}
              </div>
              {item.id === 'feedback' && unreadCount > 0 && (
                <span className="bg-[#e63946] text-white text-[9px] min-w-[18px] h-[18px] px-1 rounded-full font-black flex items-center justify-center">
                  {unreadCount}
                </span>
              )}
            </button>
        ...
        <div className="px-3 py-4 border-t border-white/[0.06] space-y-2">
          <div className="px-3 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#e63946]/30 to-[#c1121f]/20 border border-[#e63946]/30 flex items-center justify-center text-xs font-black text-[#e63946] flex-shrink-0">
                {user.name?.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <p className="text-white font-bold text-xs truncate">{user.name}</p>
                <p className="text-[#e63946] text-[9px] uppercase tracking-widest font-bold">{user.role}</p>
              </div>
            </div>
          </div>
          <button
    ...
    <header className="h-14 lg:h-16 flex items-center justify-between px-4 lg:px-8 border-b border-white/[0.06] bg-[#0c0c0f]/90 backdrop-blur-xl sticky top-0 z-40">
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="lg:hidden w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-[#9ca3af] hover:text-white transition-colors"
        >
          <Menu size={16} />
        </button>
        <div>
          <h2 className="text-sm lg:text-base font-black text-white tracking-wide">{title}</h2>
          <p className="text-[10px] text-[#4b5563] hidden sm:block">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onRefresh}
          className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-[#6b7280] hover:text-[#e63946] hover:border-[#e63946]/30 transition-all"
        >
          <RefreshCw size={14} />
        </button>
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-[#6b7280] hover:text-white hover:border-white/20 transition-all relative"
          >
            <Bell size={15} />
            {unread.length > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-[#e63946] rounded-full text-[9px] text-white flex items-center justify-center font-black px-1">
                {unread.length}
              </span>
            )}
          </button>
            ...
            {showNotifs && (
              ...
              >
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
                  ...
                  <button onClick={onMarkAllRead} className="text-[10px] text-[#e63946] hover:underline font-bold">
                    Mark all read
                  </button>
                </div>
                <div className="max-h-64 overflow-y-auto divide-y divide-white/[0.04]">
                  {notifications.length === 0 ? (
                    <p className="text-[#6b7280] text-xs text-center py-6">No notifications</p>
                  ) : notifications.slice(0, 15).map(n => (
                    <div key={n.id} className={`px-4 py-3 ${!n.is_read ? 'bg-[#e63946]/[0.04]' : ''}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-4 lg:gap-5">
        ...
        <div className={`lg:col-span-2 ${CSS.panel} p-5`}>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg bg-[#e63946]/10 border border-[#e63946]/20 flex items-center justify-center">
              <Flame size={13} className="text-[#e63946]" />
            </div>
            <h3 className="font-black text-white text-sm">Live Orders</h3>
            <span className={CSS.badge('primary')}>{liveOrders.length}</span>
          </div>
          {liveOrders.length === 0 ? (
            <p className="text-[#4b5563] text-sm text-center py-8">No active orders</p>
          ) : (
              ...
              {liveOrders.map(o => (
                <div key={o.id} className="flex items-center justify-between bg-white/[0.03] rounded-xl px-4 py-3 hover:bg-white/[0.05] transition-colors">
                  <div>
                    ...
                    <p className="text-[11px] text-[#6b7280] mt-0.5">{o.table_number ? `Table ${o.table_number}` : o.order_type} · ₹{parseFloat(o.total).toFixed(2)}</p>
                  </div>
                  <span className={CSS.badge(o.status==='pending'?'yellow':o.status==='confirmed'?'blue':'red')}>
                    {o.status}
                  </span>
                </div>
              ))}
        ...
        <div className="space-y-4">
          <div className={`${CSS.panel} p-5`}>
            ...
            {lowStock.length === 0 ? (
              <p className="text-[#4b5563] text-xs">All stock levels OK ✓</p>
            ) : (
          ...
          <div className={`${CSS.panel} p-5`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-black text-white text-sm">Top Items</h3>
            </div>
            {topItems.length === 0 ? (
              <p className="text-[#4b5563] text-xs">No data yet</p>
            ) : (
              <div className="space-y-2">
                {topItems.slice(0, 5).map((item, i) => (
                  <div key={i} className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-[#4b5563] font-black w-4">{i+1}</span>
                      <span className="text-white">{item.name}</span>
                    </div>
                    <span className="text-[#e63946] font-black">{item.count}×</span>
                  </div>
                ))}
              </div>
            )}
    ...
    <div className="space-y-4">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
              filter === f
                ...
                : 'bg-white/[0.04] text-[#6b7280] border border-white/[0.08] hover:text-white hover:border-white/20'
            }`}
          >
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-[#4b5563]">
          <OrderIcon size={32} className="mx-auto mb-3 opacity-20" />
            ...
            return (
              <div key={order.id} className={`${CSS.panelHover} p-4 lg:p-5`}>
                ...
                {order.special_instructions && (
                  <p className="text-xs text-amber-300 bg-amber-500/[0.07] border border-amber-500/20 rounded-xl px-3 py-2 mb-3">
                    📝 {order.special_instructions}
                  </p>
                )}
                ...
                <div className="flex gap-2 flex-wrap">
                  {user.role === 'captain' && !order.captain_id && (
                    <button onClick={() => claimOrder(order.id)} className={CSS.btnPrimary}>
                      ✋ Claim
                    </button>
                  )}
                  {nextStatus[order.status] && (
                    <button onClick={() => updateOrderStatus(order.id, nextStatus[order.status])} className={CSS.btnPrimary}>
                      → {nextStatus[order.status]}
                    </button>
                  )}
                  {order.status === 'served' && (
                  ...
                  {['owner','manager'].includes(user.role) && order.status !== 'paid' && (
                    <button onClick={() => deleteOrder(order.id)} className={CSS.btnDanger}>
                      <Trash2 size={12} />
                    </button>
                  )}
                </div>
        ...
        <div className={CSS.modal}>
          <div className={`${CSS.modalBox} max-w-sm`}>
            ...
            <p className="text-[#6b7280] text-sm mb-5">
              Order {paying.id} · <span className="text-[#e63946] font-black text-base">₹{parseFloat(paying.total).toFixed(2)}</span>
            </p>
              ...
              {['cash','card','upi'].map(m => (
                <button
                  ...
                  className={`py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all ${
                    payMethod === m ? 'bg-gradient-to-r from-[#e63946] to-[#c1121f] text-white shadow-lg' : 'bg-white/[0.04] text-[#6b7280] border border-white/[0.08] hover:text-white'
                  }`}
                >
                  {m}
                </button>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handlePayment(paying)} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-500 text-white text-xs font-black uppercase hover:bg-emerald-600 transition-all">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          <Search size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#4b5563]" />
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search menu..."
            className={`${CSS.input} pl-9`}
          />
        </div>
        <button
          onClick={() => { setAdding(true); setForm({ name:'', price:'', category:'Biryani', description:'', prep_time:20, spice_level:2, is_veg:false, is_halal:true, image_emoji:'🍽️', is_available:true }); }}
        ...
        {filtered.map(item => (
          <div key={item.id} className={`${CSS.panelHover} p-4`}>
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={CSS.badge(item.is_available ? 'green' : 'red')}>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setEditing(item); setForm({...item}); }} className={`flex-1 ${CSS.btnGhost} justify-center text-[10px]`}>
                <Edit2 size={11} /> Edit
              </button>
              <button onClick={() => toggleMenuAvailability(item.id)} className="flex-1 py-2 bg-amber-500/[0.07] border border-amber-500/20 text-amber-400 rounded-xl text-[10px] font-black hover:bg-amber-500/[0.12] transition-all">
                Toggle
              </button>
              <button onClick={() => handleDelete(item.id)} className={CSS.btnDanger}>
                <Trash2 size={12} />
              </button>
            </div>
          </div>
          ...
          <div className={`${CSS.modalBox} max-w-lg`}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setEditing(null); setAdding(false); }}>
                <X size={18} className="text-[#6b7280] hover:text-white" />
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[['name','Item Name','text'],['price','Price (₹)','number'],['category','Category','text'],['image_emoji','Emoji','text'],['prep_time','Prep Time (min)','number'],['spice_level','Spice (1-5)','number']].map(([k, label, type]) => (
                <div key={k}>
                  <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">{label}</label>
                  <input type={type} value={form[k]||''} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} className={CSS.input} />
                </div>
              ))}
              <div className="col-span-2">
                <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Description</label>
                <textarea value={form.description||''} onChange={e=>setForm(p=>({...p,description:e.target.value}))} rows={2}
                  className={`${CSS.input} resize-none`} />
              </div>
              <div className="flex items-center gap-3">
                <label className="text-xs font-bold text-[#6b7280]">Veg</label>
                <input type="checkbox" checked={!!form.is_veg} onChange={e=>setForm(p=>({...p,is_veg:e.target.checked}))} className="w-4 h-4 accent-[#e63946]" />
              </div>
              <div className="flex items-center gap-3">
                <label className="text-xs font-bold text-[#6b7280]">Halal</label>
                <input type="checkbox" checked={!!form.is_halal} onChange={e=>setForm(p=>({...p,is_halal:e.target.checked}))} className="w-4 h-4 accent-[#e63946]" />
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={() => { setEditing(null); setAdding(false); }} className={`flex-1 ${CSS.btnGhost} justify-center`}>Cancel</button>
              <button onClick={handleSave} disabled={saving} className={`flex-1 ${CSS.btnPrimary} justify-center`}>
...
const InventoryTab = ({ ingredients, fetchIngredients, user }) => {
  ...
  const openEdit = (ing) => {
    setForm({ name:ing.name, unit:ing.unit, stock:ing.stock, min_stock:ing.min_stock, unit_cost:ing.unit_cost||'', reorder_lead_days:ing.reorder_lead_days||2 });
    setEditModal(ing);
  };
  const openAdd = () => {
    setForm({ name:'', unit:'kg', stock:'', min_stock:'', unit_cost:'', reorder_lead_days:'2' });
    setAddModal(true);
  };
  const handleSave = async () => {
    setSaving(true);
    try {
      if (editModal) await ingredientsAPI.update(editModal.id, form);
  ...
  const handleUse = async () => {
    ...
    try {
      await ingredientsAPI.use(useModal.id, parseFloat(useAmt));
      await fetchIngredients();
  ...
  const FormModal = ({ title, onClose }) => (
    <div className={CSS.modal}>
      <div className={`${CSS.modalBox} max-w-md space-y-4`}>
        ...
        {[
          { label:'Name', key:'name', type:'text', ph:'e.g. Basmati Rice' },
          { label:'Unit', key:'unit', type:'text', ph:'kg / g / L / pcs' },
          { label:'Current Stock', key:'stock', type:'number', ph:'0' },
          { label:'Min Stock', key:'min_stock', type:'number', ph:'0' },
          { label:'Unit Cost (₹)', key:'unit_cost', type:'number', ph:'0.00' },
          { label:'Reorder Lead Days', key:'reorder_lead_days', type:'number', ph:'2' },
        ].map(f => (
          <div key={f.key}>
            <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">{f.label}</label>
            <input type={f.type} placeholder={f.ph} value={form[f.key]} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))} className={CSS.input}/>
          </div>
        ))}
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[#6b7280] text-xs mt-0.5">{ingredients.length} items · <span className="text-red-400 font-black">{low.length} low stock</span></p>
        </div>
        {isManager && (
          <button onClick={openAdd} className={CSS.btnPrimary}>
            <Plus size={13}/> Add Item
          </button>
        )}
      </div>
      ...
      <div className="flex bg-white/[0.03] border border-white/[0.06] rounded-xl p-1 gap-1 w-fit">
        <button onClick={() => setSubTab('ok')}
          className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${subTab==='ok' ? 'bg-emerald-500 text-white shadow-lg' : 'text-[#6b7280] hover:text-white'}`}>
          <span className="w-2 h-2 rounded-full bg-current"/> In Stock ({good.length})
        </button>
        <button onClick={() => setSubTab('low')}
          className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${subTab==='low' ? 'bg-red-500 text-white shadow-lg' : 'text-[#6b7280] hover:text-white'}`}>
      ...
      {list.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-[#4b5563] text-sm">{subTab==='low' ? 'All items are sufficiently stocked ✓' : 'No items found'}</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {list.map(ing => {
            ...
            return (
              <div key={ing.id} className={`${CSS.panel} p-5 transition-all ${isLow ? 'border-red-500/30' : 'hover:border-white/[0.12]'}`}>
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <p className="font-black text-white text-sm">{ing.name}</p>
                    <p className="text-[#4b5563] text-[10px] mt-0.5 uppercase tracking-widest">{ing.unit}</p>
                  </div>
                  <span className={CSS.badge(isLow ? 'red' : 'green')}>
                    {isLow ? '⚠ LOW' : '✓ OK'}
                  </span>
                </div>
                <div className="mb-3">
                  <div className="flex justify-between mb-1.5">
                    ...
                    <span className="text-[#4b5563] text-[11px]">Min: {ing.min_stock}</span>
                  </div>
                ...
                <div className="flex gap-2">
                  {isCook && (
                    <button onClick={() => { setUseModal(ing); setUseAmt(''); }}
                      className="flex-1 py-2 bg-orange-500/[0.07] border border-orange-500/20 text-orange-400 hover:bg-orange-500/[0.12] rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-1">
                      <Minus size={11}/> Use
                    </button>
                  )}
                  {isManager && (
                    <>
                      <button onClick={() => openEdit(ing)} className={`flex-1 ${CSS.btnGhost} justify-center text-[10px] py-2`}>
                        <Edit2 size={11}/> Edit
                      </button>
                      <button onClick={() => setDelConfirm(ing)} className={CSS.btnDanger}>
                        <Trash2 size={11}/>
                      </button>
                    </>
                  )}
      ...
      {useModal && (
        <div className={CSS.modal}>
          <div className={`${CSS.modalBox} max-w-sm space-y-4`}>
            <div className="flex justify-between items-center">
              <h3 className="text-white font-black">Use Ingredient</h3>
              <button onClick={() => setUseModal(null)}><X size={18} className="text-[#6b7280] hover:text-white"/></button>
            </div>
            <div className={`${CSS.panel} px-4 py-3`}>
              <p className="text-white font-black">{useModal.name}</p>
              <p className="text-[#6b7280] text-xs mt-0.5">Available: <span className="text-white font-bold">{parseFloat(useModal.stock).toFixed(1)} {useModal.unit}</span></p>
            </div>
            <div>
              <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Amount to Use ({useModal.unit})</label>
              <input type="number" min="0.01" step="0.01" value={useAmt} onChange={e => setUseAmt(e.target.value)} placeholder="0.00" className={CSS.input} autoFocus/>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setUseModal(null)} className={`flex-1 ${CSS.btnGhost} justify-center`}>Cancel</button>
              <button onClick={handleUse} disabled={!useAmt} className="flex-1 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs font-black uppercase disabled:opacity-50 transition-all">
                Confirm Use
              </button>
            </div>
          </div>
      ...
      {delConfirm && (
        <div className={CSS.modal}>
          <div className={`${CSS.modalBox} max-w-sm border-red-500/20 space-y-4`}>
            ...
            <div className="bg-red-500/[0.07] border border-red-500/20 rounded-xl px-4 py-3">
              <p className="text-white font-black">{delConfirm.name}</p>
              <p className="text-[#6b7280] text-xs mt-0.5">{delConfirm.stock} {delConfirm.unit} remaining</p>
            </div>
            <p className="text-red-400 text-xs">This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setDelConfirm(null)} className={`flex-1 ${CSS.btnGhost} justify-center`}>Cancel</button>
              <button onClick={() => handleDelete(delConfirm.id)} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-black uppercase transition-all">Delete</button>
            </div>
          </div>
        </div>
      )}
...
const FeedbackTab = () => {
  ...
  const typeColors = {
    complaint:  CSS.badge('red'),
    suggestion: CSS.badge('blue'),
    compliment: CSS.badge('green'),
    general:    CSS.badge('gray'),
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        {['all','complaint','suggestion','compliment','open','resolved'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
              filter===f ? 'bg-gradient-to-r from-[#e63946] to-[#c1121f] text-white shadow-lg shadow-red-900/20' : 'bg-white/[0.04] text-[#6b7280] border border-white/[0.08] hover:text-white'
            }`}>
            {f}
          </button>
        ))}
      ...
      {filtered.length === 0 ? (
        <p className="text-center text-[#4b5563] py-16">No feedback yet</p>
      ) : (
        <div className="grid md:grid-cols-2 gap-4">
          {filtered.map(fb => (
            <div key={fb.id} className={`${CSS.panelHover} p-5`}>
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="font-black text-white text-sm">{fb.customer_name || 'Anonymous'}</p>
                  {fb.customer_email && (
                    <a href={`mailto:${fb.customer_email}`} className="text-[#e63946] text-xs hover:underline block mt-0.5">{fb.customer_email}</a>
                  )}
                  {fb.customer_phone && (
                    <a href={`tel:${fb.customer_phone}`} className="text-[#6b7280] text-xs block">{fb.customer_phone}</a>
                  )}
                  <p className="text-[#4b5563] text-[11px] mt-1">{new Date(fb.created_at).toLocaleDateString()}</p>
                </div>
                <div className="flex flex-col gap-1.5 items-end">
                  <span className={typeColors[fb.feedback_type] || CSS.badge('gray')}>{fb.feedback_type}</span>
                  <span className={CSS.badge(fb.status==='resolved'?'green':'yellow')}>{fb.status}</span>
                </div>
              </div>
              {fb.rating && (
                <div className="flex gap-0.5 mb-2">
                  {[1,2,3,4,5].map(s => <Star key={s} size={12} className={s<=fb.rating?'text-amber-400 fill-amber-400':'text-white/10'}/>)}
                </div>
              )}
              ...
              {fb.staff_reply && (
                <div className="bg-[#e63946]/[0.07] border border-[#e63946]/20 rounded-xl p-3 text-xs text-white mb-3">
                  <span className="text-[#e63946] font-black">Reply: </span>{fb.staff_reply}
                </div>
              )}
              {fb.status !== 'resolved' && (
                <button onClick={() => setSelected(fb)} className={`${CSS.btnPrimary} text-[10px] px-3 py-2`}>
                  Reply
                </button>
              )}
            </div>
      ...
      {selected && (
        <div className={CSS.modal}>
          <div className={`${CSS.modalBox} max-w-md`}>
            ...
            <textarea
              ...
              className={`${CSS.input} resize-none mb-4`}
            />
            <div className="flex gap-3">
              <button onClick={() => setSelected(null)} className={`flex-1 ${CSS.btnGhost} justify-center`}>Cancel</button>
              <button onClick={handleReply} className={`flex-1 ${CSS.btnPrimary} justify-center`}>Send Reply</button>
            </div>
          </div>
        </div>
      )}
...
const CateringTab = () => {
  ...
  const SC = {
    submitted: { label:'Submitted', badge:'yellow' },
    reviewing: { label:'Reviewing', badge:'blue' },
    accepted:  { label:'Accepted',  badge:'green' },
    ...
    <div className="space-y-4">
      <div className="flex gap-2 overflow-x-auto pb-1">
        {['all','submitted','reviewing','accepted','rejected','completed'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex-shrink-0 ${
              filter===f ? 'bg-gradient-to-r from-[#e63946] to-[#c1121f] text-white shadow-lg shadow-red-900/20' : 'bg-white/[0.04] text-[#6b7280] border border-white/[0.08] hover:text-white'
            }`}>
      ...
      {loading
        ? <div className="flex justify-center py-12"><Loader size={24} className="animate-spin text-[#e63946]"/></div>
        : filtered.length === 0
          ? <p className="text-center py-10 text-[#4b5563] text-sm">No catering requests found</p>
          : (
            <div className="space-y-3">
              {filtered.map(req => {
                    ...
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <p className="font-black text-white">{req.customer_name}</p>
                          <span className={CSS.badge(sc.badge)}>{sc.label}</span>
                        </div>
                      ...
                      <button onClick={() => openUpdate(req)} className={`${CSS.btnPrimary} text-[10px] flex-shrink-0`}>
        ...
        <div className={CSS.modal}>
          <div className={`${CSS.modalBox} max-w-md`}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black text-white">Manage — {selected.customer_name}</h3>
              <button onClick={() => setSelected(null)}><X size={18} className="text-[#6b7280] hover:text-white"/></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Update Status</label>
                <select value={form.status} onChange={e=>setForm(p=>({...p,status:e.target.value}))} className={CSS.input}>
              ...
              <div>
                <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Quoted Amount (₹)</label>
                <input type="number" value={form.quoted_amount} onChange={e=>setForm(p=>({...p,quoted_amount:e.target.value}))} placeholder="0.00" className={CSS.input}/>
              </div>
              <div>
                <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Notes to Customer</label>
                <textarea value={form.staff_notes} onChange={e=>setForm(p=>({...p,staff_notes:e.target.value}))} rows={2} className={`${CSS.input} resize-none`} placeholder="Details, next steps..."/>
              </div>
              {form.status === 'rejected' && (
                <div>
                  <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Rejection Reason</label>
                  <textarea value={form.rejection_reason} onChange={e=>setForm(p=>({...p,rejection_reason:e.target.value}))} rows={2} className={`${CSS.input} resize-none`}/>
                </div>
              )}
              {selected.customer_email && (
                <p className="text-[10px] text-[#4b5563] flex items-center gap-1">
...
const AnnouncementsTab = ({ user }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await load();
      setForm({ title:'', message:'', type:'info', target:'all', send_email: false });
    } catch(e) { console.error(e); }
    setSaving(false);
  };
    ...
    <div className="space-y-5">
      ...
      <div className={`${CSS.panel} p-5`}>
        ...
        <div className="grid md:grid-cols-2 gap-3 mb-3">
          ...
          />
          <div className="flex gap-2">
            <select value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value}))} className={`flex-1 ${CSS.input}`}>
              {['info','promo','special','warning'].map(t => <option key={t} value={t} className="bg-gray-900">{t}</option>)}
            </select>
            <select value={form.target} onChange={e=>setForm(p=>({...p,target:e.target.value}))} className={`flex-1 ${CSS.input}`}>
              {['all','staff','customer'].map(t => <option key={t} value={t} className="bg-gray-900">{t}</option>)}
            </select>
          </div>
        </div>
        <textarea
          ...
          rows={2}
          placeholder="Message..."
          className={`${CSS.input} resize-none mb-3`}
        />
        <div className="flex items-center justify-between flex-wrap gap-3">
          <label className="flex items-center gap-2.5 cursor-pointer group">
            <div className={`w-10 h-5 rounded-full transition-colors relative ${form.send_email ? 'bg-[#e63946]' : 'bg-white/10'}`}
              onClick={() => setForm(p => ({...p, send_email: !p.send_email}))}>
            ...
            <span className="text-xs text-[#9ca3af] group-hover:text-white transition-colors">Also send email to customers</span>
          </label>
          <button onClick={handleCreate} disabled={saving} className={CSS.btnPrimary}>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-[#9ca3af] mt-1 leading-relaxed">{a.message}</p>
                    <p className="text-[10px] text-[#4b5563] mt-2 uppercase tracking-widest">
                      {ts.label} · {a.target} · {new Date(a.created_at).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                {user.role === 'owner' && (
                  <button onClick={() => handleDelete(a.id)} className="text-[#6b7280] hover:text-red-400 transition-colors flex-shrink-0 mt-0.5">
                    <X size={15}/>
                  </button>
                )}
              </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className={`${CSS.panel} px-4 py-2.5`}>
          <p className="text-[#6b7280] text-xs">Total: <span className="text-[#e63946] font-black">{totalHours.toFixed(1)} hours</span></p>
        </div>
      </div>

      {loading ? (
        <div className="flex justify-center py-16"><Loader className="animate-spin text-[#e63946]" size={24}/></div>
      ) : (
        <div className={`${CSS.panel} overflow-hidden`}>
          ...
          {shifts.length === 0 ? (
            <p className="text-center text-[#4b5563] py-10 text-sm">No shifts on this date</p>
          ) : shifts.map(s => (
            <div key={s.id} className={`${CSS.tableRow} grid-cols-5`}>
              <p className="font-black text-white text-xs">{s.users?.name||'—'}</p>
              <span className={CSS.badge('primary')}>{s.users?.role||'—'}</span>
              <p className="text-[#9ca3af] text-xs">{s.clock_in ? new Date(s.clock_in).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}) : '—'}</p>
              <p className="text-[#9ca3af] text-xs">{s.clock_out ? new Date(s.clock_out).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}) : <span className="text-amber-400 font-bold">Active</span>}</p>
              <p className="text-white font-black text-xs">{s.duration_minutes ? `${Math.floor(s.duration_minutes/60)}h ${s.duration_minutes%60}m` : '—'}</p>
            </div>
          ))}
        </div>
      )}
...
const TablesTab = ({ user }) => {
  ...
  const load = () => {
    import('../api/index.js').then(({ tablesAPI }) =>
      tablesAPI.getAll().then(r => { setTables(r.data); setLoading(false); }).catch(() => setLoading(false))
    );
  };
  useEffect(() => { load(); }, []);
  ...
  const handleSave = async () => {
    ...
    const { tablesAPI } = await import('../api/index.js');
    try {
      if (editModal) await tablesAPI.update(editModal.id, form);
      else await tablesAPI.create(form);
      load(); setAddModal(false); setEditModal(null);
    } catch(e) { alert(e.response?.data?.error || 'Failed'); }
    finally { setSaving(false); }
  };
  const handleDelete = async (id) => {
    const { tablesAPI } = await import('../api/index.js');
    try { await tablesAPI.delete(id); load(); setDelConfirm(null); }
    catch(e) { alert(e.response?.data?.error || 'Delete failed'); }
  };
  ...
  const FormModal = ({ title, onClose }) => (
    <div className={CSS.modal}>
      <div className={`${CSS.modalBox} max-w-sm space-y-4`}>
        ...
        {[
          { label:'Table Number *', key:'table_number', type:'text',   ph:'e.g. T11' },
          { label:'Capacity',       key:'capacity',     type:'number', ph:'4' },
        ].map(f => (
          <div key={f.key}>
            <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">{f.label}</label>
            <input type={f.type} value={form[f.key]} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))} placeholder={f.ph} className={CSS.input}/>
          </div>
        ))}
        <div>
          <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Section</label>
          <select value={form.section} onChange={e=>setForm(p=>({...p,section:e.target.value}))} className={CSS.input}>
            {['main','indoor','outdoor','private','bar','banquet'].map(s=><option key={s} value={s} className="bg-gray-900">{s}</option>)}
          </select>
        </div>
        {editModal && (
          <div>
            <label className="text-[10px] font-black text-[#4b5563] uppercase tracking-widest block mb-1">Status</label>
            <select value={form.status} onChange={e=>setForm(p=>({...p,status:e.target.value}))} className={CSS.input}>
              {['available','occupied','reserved','cleaning'].map(s=><option key={s} value={s} className="bg-gray-900">{s}</option>)}
            </select>
          </div>
        )}
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className={`flex-1 ${CSS.btnGhost} justify-center`}>Cancel</button>
          <button onClick={handleSave} disabled={saving || !form.table_number} className={`flex-1 ${CSS.btnPrimary} justify-center`}>
            {saving ? <Loader size={13} className="animate-spin"/> : <Save size={13}/>}
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-black text-white">Table Status</h2>
          <p className="text-[#6b7280] text-xs mt-0.5">{tables.length} tables total</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label:'Available', count:counts.available, badge:'green' },
          { label:'Occupied',  count:counts.occupied,  badge:'red' },
          { label:'Reserved',  count:counts.reserved,  badge:'blue' },
          { label:'Cleaning',  count:counts.cleaning,  badge:'yellow' },
        ].map(s => (
          <div key={s.label} className={`${CSS.panel} p-3 text-center`}>
            <p className={`text-2xl font-black ${
              s.badge==='green'?'text-emerald-400':s.badge==='red'?'text-red-400':s.badge==='blue'?'text-sky-400':'text-amber-400'
            }`}>{s.count}</p>
            <p className="text-[10px] text-[#6b7280] uppercase tracking-widest mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {tables.map(t => {
          ...
          return (
            <div key={t.id} className={`${sc.bg} border ${sc.border} rounded-2xl p-4 transition-all`}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-xl font-black text-white">{t.table_number}</p>
              ...
              <select
                ...
                className="w-full bg-black/30 border border-white/10 rounded-lg px-2 py-1.5 text-[10px] text-white focus:outline-none focus:border-white/30 mb-2"
              >
                {['available','occupied','reserved','cleaning'].map(s=><option key={s} value={s}>{s}</option>)}
              </select>
              {isManager && (
                <div className="flex gap-1.5">
                  <button onClick={() => openEdit(t)} className="flex-1 py-1.5 bg-white/[0.06] hover:bg-white/[0.10] rounded-lg text-[10px] font-bold text-[#9ca3af] hover:text-white transition-all flex items-center justify-center gap-1">
                    <Edit2 size={10}/> Edit
                  </button>
                  <button onClick={() => setDelConfirm(t)} className={CSS.btnDanger + ' py-1.5 px-2'}>
                    <Trash2 size={10}/>
                  </button>
                </div>
              )}
      ...
      {delConfirm && (
        <div className={CSS.modal}>
          <div className={`${CSS.modalBox} max-w-sm border-red-500/20 space-y-4`}>
            <h3 className="text-white font-black">Delete Table?</h3>
            <div className="bg-red-500/[0.07] border border-red-500/20 rounded-xl px-4 py-3">
              <p className="text-white font-black">Table {delConfirm.table_number}</p>
              <p className="text-[#6b7280] text-xs mt-0.5">{delConfirm.capacity} seats · {delConfirm.section}</p>
            </div>
            <p className="text-red-400 text-xs">This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setDelConfirm(null)} className={`flex-1 ${CSS.btnGhost} justify-center`}>Cancel</button>
...
const ReservationsTab = () => {
  ...
  const statusBadge = {
    pending:'yellow', confirmed:'blue', seated:'green', completed:'primary', cancelled:'red'
  };
  ...
  return (
    <div className="space-y-3">
      {reservations.length === 0 ? (
        <p className="text-center text-[#4b5563] py-16">No reservations found</p>
      ) : reservations.map(r => (
        <div key={r.id} className={`${CSS.panelHover} p-4 lg:p-5`}>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="font-black text-white">{r.customer_name}</p>
              <p className="text-[#6b7280] text-xs mt-0.5">{r.customer_phone}{r.customer_email ? ` · ${r.customer_email}` : ''}</p>
              <div className="flex flex-wrap gap-4 mt-2 text-xs text-[#6b7280]">
                <span><Calendar size={10} className="inline mr-1 text-[#e63946]"/>{r.reservation_date}</span>
                <span><Clock size={10} className="inline mr-1 text-[#e63946]"/>{r.reservation_time}</span>
                <span><Users size={10} className="inline mr-1 text-[#e63946]"/>{r.party_size} guests</span>
              </div>
              {r.special_requests && <p className="text-amber-300 text-xs mt-1">📝 {r.special_requests}</p>}
            </div>
            <div className="flex flex-col gap-2 items-end">
              <span className={CSS.badge(statusBadge[r.status] || 'gray')}>{r.status}</span>
              <select
                value={r.status}
                onChange={e => updateStatus(r.id, e.target.value)}
                className="bg-white/[0.04] border border-white/[0.08] rounded-lg px-2 py-1 text-[10px] text-white focus:outline-none"
              >
...
const ProfileTab = ({ user }) => {
  ...
  const handleClockIn  = async () => { const r = await shiftsAPI.clockIn(); setActiveShift(r.data); };
  const handleClockOut = async () => { if (!activeShift) return; await shiftsAPI.clockOut(activeShift.id, ''); setActiveShift(null); };
    ...
    <div className="max-w-sm mx-auto space-y-4">
      ...
      <div className={`${CSS.panel} p-5`}>
        <h3 className="font-black text-white text-sm mb-4 flex items-center gap-2">
          <Clock size={14} className="text-[#e63946]"/> Shift Management
        </h3>
        {activeShift ? (
          <div className="space-y-3">
            <p className="text-emerald-400 text-sm font-bold">✓ Clocked in since {new Date(activeShift.clock_in).toLocaleTimeString()}</p>
            <button onClick={handleClockOut} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl text-sm font-black uppercase transition-all">Clock Out</button>
          </div>
        ) : (
          <button onClick={handleClockIn} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-black uppercase transition-all">Clock In</button>
        )}
      </div>
    </div>
  );
...
const Dashboard = () => {
  ...
  const claimOrder = async (orderId) => {
    try { await ordersAPI.claim(orderId); fetchOrders(); }
  ...
  const tabTitles = {
    ...
    shifts:'Shift Logs', staff:'Staff Management',
  };
